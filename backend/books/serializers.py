...
class BookSerializer(serializers.ModelSerializer):
    ...
    def validate_isbn(self, value):
        """
        Check that ISBN is valid and unique.
        """
        # For simplicity, we just check length here, but in a real application
        # we'd validate the ISBN format and check digit
        if len(value) != 13 and len(value) != 10:
            raise serializers.ValidationError("ISBN must be 10 or 13 characters long")
        
        # Check uniqueness for new books or when ISBN is changed
    ...
    def validate_copies_available(self, value):
        """
        Check that copies_available is a positive integer.
        """
        if value < 0:
